...
| **DP-F · Gamificação que acolhe** | Fase 2/3 | 🟡 mockup aprovado; Fase 0/1/2 feitas; falta Fase 3 (UI: aba Jornada + planta SVG + animações + desafio) |
...
- **30/08/2026** — **Fase 2** (motor da gamificação, sem UI): `S.progress` + `users/{uid}.progress`; 8 estágios da planta por "dias de cuidado"; 12 conquistas + `checkBadges`; `markCareDay` com hooks em registro/gratidão/hábito/respiração; conquista Recomeço por ausência de 7+ dias; contadores de respiração e SOS. Enviado para `origin/main`. Próximo: **Fase 3** (aba Jornada + planta SVG + animações + desafio semanal).
