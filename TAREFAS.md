...
- **19/09/2026** — **Bloco 17 (segurança da conta):** bloqueio do app com código de 6 dígitos (PBKDF2, 200 mil rodadas, sal aleatório) e digital pelo plugin `@capgo/capacitor-native-biometric`; espera crescente depois de 5 e 8 erros; escolha de quando bloquear. **Alterar senha** pelo link de redefinição. **Excluir minha conta dentro do app**, com confirmação de identidade e a função `netlify/functions/excluir-conta.mjs` apagando dados, conta e enviando o e-mail. Mensagens de login deixaram de revelar se um e-mail tem conta. **Teste de invasão das regras do Firestore** (`npm run teste:regras`): 44 casos, 35 ataques recusados. Conferido na API do Firebase que a proteção contra descoberta de contas já está ativa no projeto. **Pendente do André:** criar a conta no Resend, gerar a chave de conta de serviço do Firebase e cadastrar `FIREBASE_SERVICE_ACCOUNT`, `RESEND_API_KEY` e `EMAIL_REMETENTE` nas variáveis do Netlify — sem isso a exclusão responde erro. Enquanto não houver domínio próprio verificado no Resend, o e-mail de confirmação só chega no endereço do dono da conta.
- **19/09/2026** — **Exclusão de conta no ar e testada de ponta a ponta.** A função do Netlify foi refeita sem o SDK `firebase-admin` (ele quebrava em produção com 502: `jwks-rsa` chama `jose`, que é ESM, e o pacote sai CommonJS) — agora assina o JWT da conta de serviço com o `crypto` do Node e fala com Firestore e Identity Toolkit por REST, como as funções do Bússola. Teste real: conta criada, dados gravados em 7 coleções, exclusão chamada, tudo apagado, conta removida do Auth e e-mail entregue. Corrigida a contagem, que inflava porque o Firestore aceita apagar documento inexistente (agora com `currentDocument.exists=true`). Remetente: `naoresponda.noreplymail@gmail.com`.
- **19/09/2026** — **Bloco 18:** convite "plante uma semente" (cartão desenhado no canvas + frase do mural + link para `site/convite/`, que dá para trocar sem gerar pacote) e boas-vindas animadas em 4 cenas depois do cadastro, com Pular e opção de rever em Você. Versão 0.10.0 (versionCode 4). **Lição do build:** o JDK do Android Studio virou Java 25 e derruba o Gradle (`Unsupported class file major version 69`); use `JAVA_HOME` apontando para `C:\Program Files\Java\jdk-21`.
- **20/09/2026** — **Travamento ao entrar, corrigido.** O app esperava toda a sincronização da nuvem antes de abrir a tela (perfil + 5 coleções em sequência); com rede ruim, ficava parado na entrada — foi o que o André relatou como "trava ao entrar" e "trava quando entro com a senha". Agora a tela abre com os dados do aparelho (o app é offline-first) e a sincronização roda em segundo plano, redesenhando a tela quando chega. Toda espera de rede no caminho de entrada ganhou prazo (4 s para o perfil, 25 s para autenticar e sincronizar) e a mensagem de erro passou a falar de conexão, não de senha. No cadastro, a gravação do perfil no banco deixou de segurar a conclusão. **Criada a [Carta de Navegação v2.0](CARTA-DE-NAVEGACAO.md)** — ela existia só fora do computador; agora é versionada junto do código. Lições viraram política nas habilidades: espera de rede fora do caminho da primeira tela (`references/app-que-nao-trava.md`), JDK 21 para o Gradle e documentos obrigatórios de cada projeto.
- **21/09/2026** — **Bloco 19: vocabulário que aprende com as pessoas.** As emoções passaram de 24 para 48 (12 por tom, 6 à mostra e o resto em "mais palavras") — a granularidade emocional é o próprio mecanismo do app: quanto mais precisa a palavra, melhor a pessoa lida com o que sente. Quem não encontra a palavra escreve a dela (até 12 por pessoa, a menos usada sai); a palavra vira etiqueta pessoal, sincroniza com a conta e, se as estatísticas anônimas estiverem ligadas, é contada num acervo anônimo (`vocab`): só a palavra, o tom e um contador, sem uid e sem o registro. O administrador aprova na aba **Palavras** do painel e a palavra entra na roda de todo mundo **sem pacote novo**. Regras novas para `vocab` com 12 casos no teste de invasão (56 no total, todos passando). Leitura nova "Estresse, sono e intimidade" — o tema entra como leitura e o app **não registra nada** sobre vida sexual, que é dado sensível na LGPD. **Ação do André:** publicar o `firestore.rules` antes de subir o pacote, senão a sugestão de palavra falha em silêncio.
