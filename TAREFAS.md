...
- [~] **[F2.4]** `P0` Login Google: web usa `signInWithPopup`. **Falta o plugin nativo** (`@capacitor-firebase/authentication`) para o Android. **Bloqueado por você:** no Firebase Console → adicionar app Android `br.com.florescer.app` → colar as SHA (**debug SHA-1** `FE:DD:D7:1E:44:18:03:6F:8A:B5:68:E8:70:77:E1:20:05:1A:36:8A`) → baixar `google-services.json` em `android/app/`. Depois eu ligo plugin + gradle + código. _(depende de: F2.3 ✅)_
